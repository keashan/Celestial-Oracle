      ...
      <div className="glass p-14 md:p-20 rounded-[4rem] relative overflow-hidden border border-white/10 shadow-2xl">
        ...
        <div className="relative z-10">
          <div className="flex items-center space-x-10 mb-12">
            ...
            <h2 className="text-[length:var(--fs-heading-main)] font-bold text-white tracking-tight leading-none">{prediction.zodiacSign}</h2>
          </div>
          <p className="text-white/90 text-[length:var(--fs-heading-sub)] leading-relaxed mb-12 max-w-4xl font-light">
            {intro}
          </p>
          <div className="border-l-[12px] border-purple-500/30 pl-12 my-14 bg-white/5 py-14 rounded-r-[4rem]">
            <p className="text-[length:var(--fs-prediction-quote)] italic text-purple-50 font-light leading-snug">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {prediction.monthlyBreakdown.map((item, index) => (
          <div key={index} className="glass p-12 rounded-[3rem] hover:bg-white/10 transition-all border border-white/10 hover:scale-[1.04] duration-300 group shadow-xl">
            <h3 className="text-purple-400 font-bold text-[length:var(--fs-btn-text)] mb-6 group-hover:text-purple-300 transition-colors uppercase tracking-[0.25em]">{item.month}</h3>
            <p className="text-white/90 text-[length:var(--fs-body)] leading-relaxed font-light">{item.highlight}</p>
          </div>
        ))}
      </div>
    </div>
